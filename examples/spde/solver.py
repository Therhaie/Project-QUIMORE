...
class SteadyStateHeat2DSolver(object):
    ...
    def __init__(self, nx=100, ny=100, value_left=1.,
        ...
        #define mesh
        self.mesh = fipy.Grid2D(nx=self.nx, ny=self.ny, dx=self.dx, dy=self.dy) # création du maillage
        print('value of mesh',self.mesh)

        #get the location of the middle of the domain
        #cellcenters=np.array(self.mesh.cellCenters).T
        #x=cellcenters[:, 0]
        #y=cellcenters[:, 1]
        x, y = self.mesh.cellCenters # dimension (2, 1024)
        x_all=x[:self.nx] 
        # print('value of mesh.cellCenters',self.mesh.cellCenters)
        # print('dim of mesh.cellCenters',self.mesh.cellCenters.shape)
        # print('value of x',x)
        # print('value of x_all',x_all)
        y_all=y[0:-1:self.ny]
        loc1=x_all[int((self.nx-1)/2)] # value of the center of the domain in the x direction
        loc2=y_all[int((self.ny-1)/2)] # value of the center of the domain in the y direction
        self.loc=np.intersect1d(np.where(x==loc1)[0], np.where(y==loc2)[0])[0] # tuple with center of the domain
        ...
        self.eq = -fipy.DiffusionTerm(coeff=self.C) == self.source #equation to solve
