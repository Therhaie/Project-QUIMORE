...
#         self.ds = [self.D] + [int(np.ceil(self.D * np.exp(self.rho * i))) for i in range(1, self.L + 1)] + [300 * self.d, 1]
...
# def create_model(D, L, d, baselr=1e-3, lmbda=1e-6):
#     model = DeepUQSurrogate(D, L, d, baselr, lmbda)
...
# # Example usage
# D, L, d = 10, 3, 50  # Define dimensions
# x = torch.randn((32, D))  # Example input data
# y_true = torch.randn((32, 1))  # Example output data

# model, optimizer, criterion = create_model(D, L, d)
